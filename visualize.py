...
def purity(name, model, test_sig, test_bkg):
    import matplotlib.pyplot as plt
    import numpy as np
    ...
    B, binB, _ = plt.hist(test_bkg_pred, bins=100, range=(0, 1), normed=True)
    S, binS, _  = plt.hist(test_sig_pred, bins=100, range=(0, 1), normed=True)
    pure = S/np.sqrt(B)

    plt.clf()
    plt.hist(pure, histtype='stepfilled', color='red', label='Purity', bins=500, range=(0,2), normed=False, alpha=0.5)
    plt.legend()
    plt.savefig('plots/{}.png'.format(name))
